...
async function deleteDepartment() {
    ...
    const choices = queryResponse.rows.map(department => {
        return {
    ...
    inquirer.prompt([
        ...
        .then(async resp => {
            ...
            for (let i = 0; i < choices.length; i++) {
                if (resp.departmentID === choices[i].value) {
                    departmentName = choices[i].name;
                }
            }
...
async function updateDepartment() {
    ...
    inquirer.prompt([
        ...
        .then(async resp => {
            await db.updateDepartment(resp.departmentID, resp.newName);
            console.log('\n');
...
async function deleteRole() {
    ...
    const choices = queryResponse.rows.map(role => {
        return {
            name: role.title,
            value: role.role_id
        }
    ...
    inquirer.prompt([
        ...
        .then(async resp => {
            let roleName;
            for (let i = 0; i < choices.length; i++) {
                if (resp.roleID === choices[i].value) {
                    roleName = choices[i].name;
                }
            }
...
async function viewEmployeesByManager() {
    ...
    const choices = queryResponse.rows.map(manager => {
        return {
    ...
    inquirer.prompt([
        {
            name: 'managerID',
            message: 'Which managers reports would you like to see?',
            type: 'list',
            choices: choices
        },
    ])
...
async function addEmployee() {
    ...
    const roleChoices = roleQueryResponse.rows.map(role => {
        return {
            name: role.title,
            value: role.role_id
        }
    ...
    const managerChoices = managerQueryResponse.rows.map(employee => {
        return {
...
async function deleteEmployee() {
    ...
    const choices = queryResponse.rows.map(employee => {
        return {
            name: `${employee.first_name} ${employee.last_name}`,
            value: employee.id
        }
    ...
    inquirer.prompt([
        ...
        .then(async resp => {
            let employeeName;
            for (let i = 0; i < choices.length; i++) {
                if (resp.employeeID === choices[i].value) {
                    employeeName = choices[i].name;
                }
            }
            await db.deleteEmployee(resp.employeeID);
            console.log('\n');
            console.warn(`Employee ${employeeName} deleted successfully!`);
            console.log('\n');
        })
        .then(() => loadMainPrompts());
}
...
async function changeEmployeeName() {
    ...
    inquirer.prompt([
        ...
        .then(async resp => {
            ...
            console.log('\n');
            console.warn(`The employees name has been changed to ${resp.newFirstName} ${resp.newLastName} successfully!`);
            console.log('\n');
        })
        .then(() => loadMainPrompts());
}
    ...
    inquirer.prompt([
        ...
        .then(async resp => {
            ...
            for (let i = 0; i < roleChoices.length; i++) {
                if (resp.newRole === roleChoices[i].value) {
                    roleName = roleChoices[i].name;
                }
            }
            await db.changeEmployeeRole(resp.employeeID, resp.newRole);
            console.log('\n');
            console.warn(`Employee ${employeeName}s role has been changed to ${roleName} successfully!`);
            console.log('\n');
        })
        .then(() => loadMainPrompts());
}
    ...
    inquirer.prompt([
        ...
        .then(async resp => {
            ...
            for (let i = 0; i < managerChoices.length; i++) {
                if (resp.managerID === managerChoices[i].value) {
                    managerName = managerChoices[i].name;
                }
            }
            await db.changeEmployeeManager(resp.employeeID, resp.managerID);
            console.log('\n');
            console.warn(`Employee ${employeeName}s manager has been changed to ${managerName} successfully!`);
            console.log('\n');
        })
        .then(() => loadMainPrompts());
}
